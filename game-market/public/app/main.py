...
# Nota: La carga debe ser absoluta respecto al archivo .env
load_dotenv(os.path.join(os.path.dirname(__file__), '..', '.env'))

# ------------------------------------------------ #
# 🔑 FUNCIÓN CLAVE: Envuelve la aplicación para Pytest
# ------------------------------------------------ #
def create_app(test_config=None):
    app = Flask(__name__)

    # 🔑 CONFIGURACIÓN DE APP
    # Usa la clave secreta de la configuración de prueba o del entorno
    app.config['SECRET_KEY'] = test_config.get('SECRET_KEY') if test_config and 'SECRET_KEY' in test_config else os.getenv('SUPER_KEY', 'change-me')
    ...
    # ---------------- RUTAS (Endpoints) ---------------- #
    # TODAS TUS RUTAS ORIGINALES VAN DENTRO DE ESTA FUNCIÓN
...
if __name__ == "__main__":
    # Creamos la app para ejecución normal, cargando la SECRET_KEY
    app = create_app({'SECRET_KEY': os.getenv('SUPER_KEY', 'change-me')})
    # Ejecutamos la app con las configuraciones por defecto
